      ...
      .on('data', async (row) => {
        try {
          ...
          for (let i = 1; i <= 46; i++) {
            ...
            const issuedDate = handleNull(row[`issueddatetopic${i}`]);
            // topicName
            if (topicName && issuedDate) {
              console.log('topicName', topicName, issuedDate)
          ...
          for (let i = 0; i <= 61; i++) {
            ...
            const trainerIssueDate = handleNull(row[`trainer${i}issuedate`]);
            if (trainerTopicName && trainerIssueDate) {
              console.log('trainertopicname', trainerTopicName)
...
// Usage
const filePath = '/home/lucky/Downloads/full_details.csv';
importCsv(filePath);
